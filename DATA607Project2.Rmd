...
The MTA data is in a tidy format where each row represents one observation of ridership data for a specific transportation mode on a specific date.

```{r}
library(tidyr)

# Reshape data to long format
tidy_MTA_data <- MTA_data %>%
  select(Date, starts_with("Subways_Total_Estimated_Ridership"), starts_with("Buses_Total_Estimated_Ridership"), starts_with("LIRR_Total_Estimated_Ridership"), starts_with("Metro_North_Total_Estimated_Ridership")) %>%
  pivot_longer(cols = -Date, names_to = "Transportation_Mode", values_to = "Ridership")

# Remove NA values
tidy_MTA_data <- tidy_MTA_data %>% na.omit()

# Print tidy data
print(head(tidy_MTA_data))
...
I transformed the data by removing empty cells especially columns. To add on, I remove columns and rows more than 50% missing values. I also removed other columns that were redundant and therefore not needed.
...
tidy_nobel_data <- noble_peace_prize_data %>%
  select(awardYear, category, gender, prizeAmount)

# Remove NA values
tidy_nobel_data <- tidy_nobel_data %>% na.omit()

# Print tidy data
print(head(tidy_nobel_data))
```
...
I transformed the data by removing unwanted columns and renaming columns to organize the features present in the data.
```{r}
...
### Tidying the Data
```{r}
# Filter data for entire home/apartment listings
entire_home_apartment_data <- NYC_airbnb_data %>%
  filter(room_type == "Entire home/apt")

# Tidy data
tidy_NYC_airbnb_data <- entire_home_apartment_data %>%
  select(name, neighbourhood_group, neighbourhood, price, room_type)  

# Remove NA values
tidy_NYC_airbnb_data <- tidy_NYC_airbnb_data %>% na.omit()

# Print tidy data
print(head(tidy_NYC_airbnb_data))

```

### Conclusion

Throughout this project, I gained extensive experience working with untidy data and utilizing R programming along with data manipulation libraries such as tidyr and dplyr to transform the data into a tidy format. The three datasets that I worked with have been successfully tidied by reshaping them into a longer format, ensuring that each observation corresponds to a single row. Additionally, I conducted transformations to remove unnecessary columns, rename certain columns for clarity, and eliminate empty columns and rows. By leveraging the capabilities of tidyr and dplyr, I grouped the datasets based on relevant features, facilitating a clearer understanding of the data and enabling more insightful analysis. The transformation process culminated in tidy datasets that were conducive to generating meaningful visualizations using ggplot. In summary, the process of tidying the data not only improved its organization and structure but also facilitated the extraction of valuable insights and trends, thereby enhancing the overall analysis process 
